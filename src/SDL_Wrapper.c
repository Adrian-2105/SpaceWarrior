...
static SDL_Wrapper pantalla;


// FUNCIONES PRIVADAS

// Función privada para gestión de errores SDL.
void SDL_Wrapper_Error()
{
    printf("%s\n", SDL_GetError());
    exit(EXIT_FAILURE);
}
...
void SDL_Wrapper_Crea(char titulo[], int w, int h)
{
    ...
    if (SDL_SetRenderDrawBlendMode(pantalla.renderer, SDL_BLENDMODE_BLEND)==-1)
        SDL_Wrapper_Error();
...
{
    if (SDL_UpdateWindowSurface(pantalla.window)==-1)
        SDL_Wrapper_Error();
}

int SDL_Wrapper_TeclaPulsada(SDL_Scancode scancode) //SDL_Keycode keycode)
{
...
{
    if (SDL_SetRenderDrawColor(pantalla.renderer, r,g,b, a)==-1)
        SDL_Wrapper_Error();
    if (SDL_RenderClear(pantalla.renderer)==-1)
        SDL_Wrapper_Error();
}

void SDL_Wrapper_DibujaPunto(double x, double y)
{
    if (SDL_SetRenderDrawColor(pantalla.renderer, pantalla.strokeColor.r, pantalla.strokeColor.g, pantalla.strokeColor.b, pantalla.strokeColor.a)==-1)
        SDL_Wrapper_Error();
    if (SDL_RenderDrawPoint(pantalla.renderer, x,y)==-1)
        SDL_Wrapper_Error();
}
...
void SDL_Wrapper_DibujaLinea(double x1, double y1, double x2, double y2)
{
    if (SDL_SetRenderDrawColor(pantalla.renderer, pantalla.strokeColor.r, pantalla.strokeColor.g, pantalla.strokeColor.b, pantalla.strokeColor.a)==-1)
        SDL_Wrapper_Error();
    if (SDL_RenderDrawLine(pantalla.renderer, x1,y1, x2,y2)==-1)
        SDL_Wrapper_Error();
}

void SDL_Wrapper_DibujaTriangulo(double x1, double y1, double x2, double y2, double x3, double y3)
{
    if (SDL_SetRenderDrawColor(pantalla.renderer, pantalla.fillColor.r, pantalla.fillColor.g, pantalla.fillColor.b, pantalla.fillColor.a)==-1)
        SDL_Wrapper_Error();
    ...
    while ( y < y2 )
    {
        if (SDL_RenderDrawLine(pantalla.renderer,xl, y, xr, y)==-1)
            SDL_Wrapper_Error();
        xl += dxl;
        xr += dxr;
        y = y + 1;
    ...
    if (SDL_RenderDrawLine(pantalla.renderer, x1,y1, x2,y2)==-1)
        SDL_Wrapper_Error();
    if (SDL_RenderDrawLine(pantalla.renderer, x2,y2, x3,y3)==-1)
        SDL_Wrapper_Error();
    if (SDL_RenderDrawLine(pantalla.renderer, x3,y3, x1,y1)==-1)
        SDL_Wrapper_Error();
}
...
void SDL_Wrapper_DibujaRectangulo(double x, double y, double w, double h)
{
    ...
    if (SDL_SetRenderDrawColor(pantalla.renderer, pantalla.fillColor.r, pantalla.fillColor.g, pantalla.fillColor.b, pantalla.fillColor.a)==-1)
        SDL_Wrapper_Error();
    if (SDL_RenderFillRect(pantalla.renderer, &r)==-1)
        SDL_Wrapper_Error();
    if (SDL_SetRenderDrawColor(pantalla.renderer, pantalla.strokeColor.r, pantalla.strokeColor.g, pantalla.strokeColor.b, pantalla.strokeColor.a)==-1)
        SDL_Wrapper_Error();
    if (SDL_RenderDrawRect(pantalla.renderer, &r)==-1)
        SDL_Wrapper_Error();
}
...
void SDL_Wrapper_DibujaCirculo(double x, double y, double r)
{
    ...
    for (dy = 0; dy <= r; dy++)
    {
        ...
        {
            if (SDL_RenderDrawPoint(pantalla.renderer, px,y + r - dy)==-1)
                SDL_Wrapper_Error();
            if (SDL_RenderDrawPoint(pantalla.renderer, px,y - r + dy-1)==-1)
                SDL_Wrapper_Error();
        }
    }
    ...
    while (px >= py)
    {
        if (SDL_RenderDrawPoint(pantalla.renderer, cx + px, cy + py)==-1)
            SDL_Wrapper_Error();
        if (SDL_RenderDrawPoint(pantalla.renderer, cx + py, cy + px)==-1)
            SDL_Wrapper_Error();
    ...
    while (st[i])
    {
        ...
        for ( int fila = 0 ; fila < 13 ; fila++ )
        {
            ...
            for ( int columna = 0 ; columna < 8 ; columna++ )
            {
                ...
                {
                    if (SDL_RenderDrawPoint(pantalla.renderer, x+i*9+columna,y+13-fila)==-1)
                        SDL_Wrapper_Error();
                }
                mascara /= 2;
            }
...
char *SDL_Wrapper_LeeTexto(int x, int y, int max, char *titulo)
{
    ...
    while (!quit)
    {
        ...
        msg[max] = '\0';
        SDL_Wrapper_ColorRelleno(192,192,192, 255);
        SDL_Wrapper_ColorTrazo(64,64,64,255);
        SDL_Wrapper_DibujaRectangulo( x,y, (max+1)*9, 40 );

        SDL_Wrapper_ColorRelleno(255,255,255, 255);
        SDL_Wrapper_ColorTrazo(128,128,128, 255);
        SDL_Wrapper_DibujaRectangulo(x+1,y+20, (max+1)*9-2, 19);
        SDL_Wrapper_ColorTrazo(0,0,0, 255);
        SDL_Wrapper_DibujaTexto(titulo, x+4,y+2);
        SDL_Wrapper_ColorTrazo(0,0,0, 255);
        SDL_Wrapper_DibujaTexto(msg, x+4,y+22);
        tick++;
        if (tick < 20)
        {
            SDL_Wrapper_DibujaLinea(x + 4 + cursor*9, y + 33 + 1, x + 4 + cursor*9, y + 23 );
        }
        if (tick == 25)
        {
            tick = 0;
...
void SDL_Wrapper_DibujaImagen(Imagen imagen, double x, double y, double w, double h)
{
    ...
    if (SDL_BlitScaled(imagen->surface, &src, pantalla.surface, &dest)==-1)
        SDL_Wrapper_Error();
}
